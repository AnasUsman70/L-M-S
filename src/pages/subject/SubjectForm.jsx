...
function SubjectForm() {
  ...
  return (
    <div className='max-w-[600px] m-auto py-10'>
      ...
      <div>
      <h4>Gender</h4> 
      <CusRadio value={formData.group}  label="Pre-Medical" name='group' setFormData={(e)=>setFormData({...formData,group:e.target.value})}/>  
      <CusRadio value={formData.group} label="Pre-Engineering" name='group' setFormData={(e)=>setFormData({...formData,group:e.target.value})}/>  
      </div>  

      <button disabled={buttonDisable}  className={`bg-green-600 text-white py-1 rounded-sm ${buttonDisable ? "bg-green-200 cursor-not-allowed":""}`}>Sumbit</button>
      </form>
    </div>
  )
}
